$(function () {
            ...
            for (var j = date_count; j <= mGetDate(year, sarr[i]); j++) {
                if (k % 7 == 0) {
                    if (flag) {
                        ...
                        tbodyStr = tbodyStr.substring(0, index) + "<td rowspan='" + m_l_row + "' class='" + desccalss+"'  data-month='" + sarr[i - 1] +"'><div class='desc-text-wrap'><span>点击输入校历说明</span></div><div class='desc-shade'><i class='layui-icon'>&#xe642;</i>编辑</div><div class='desc-textarea layui-hide'><div class='desc-textarea-body-wrap'><textarea placeholder='请输入校历说明...'></textarea></div><div class='desc-textarea-footer-wrap'><div class='desc-textarea-footer'><button class='btn btn-important block ' type='button'>保存</button></div></div></div></td></tr>" + tbodyStr.substring(index, tbodyStr.length);
        ...
        own.ajax.post("GetSchoolCalendar", {
            ...
            for (var i = 0; i < dataArr.length; i++) {
                if (dataArr[i].jw_day) {
                    ...
                    if (d_html.length > 1) {
                        $(d_html[0]).find("span:last").text(dataArr[i].jw_note);//备注赋值
                        var t_day = new Date(year, dataArr[i].jw_month - 1, dataArr[i].jw_day);
                        for (var t = 1; t < d_html.length; t++) {
                            t_day.setDate(t_day.getDate() + 1);
                            $(d_html[t]).attr("data-month", t_day.getMonth() + 1);
                            $(d_html[t]).attr("data-day", t_day.getDate());
                            $(d_html[t]).attr("data-note", "");
                            $(d_html[t]).attr("data-contentType", "");
                            if (t_day.getDay() == 0 || t_day.getDay() == 6) {
                                $(d_html[t]).css("color", type_color[4]);//改变颜色
                                $(d_html[t]).css("background-color", type_bgcolor[0]);
                            } else {
                                $(d_html[t]).css("color", type_color[0]);//改变颜色
                                $(d_html[t]).css("background-color", type_bgcolor[0]);
                            }
                    ...
                    if (dataArr[i].jw_contentType == 0) {
                        let tt_day = new Date(year, dataArr[i].jw_month - 1, dataArr[i].jw_day);
                        if (tt_day.getDay() == 0 || tt_day.getDay() == 6) {
                            $(d_html[0]).css("color", type_color[4]);//改变颜色
                            $(d_html[0]).css("background-color", type_bgcolor[0]);
                        } else {
                            $(d_html[0]).css("color", type_color[0]);//改变颜色
                            $(d_html[0]).css("background-color", type_bgcolor[0]);
                        }
                    }
    ...
    $(".datetext").off().on("click", function () {
        ...
        var h = $(".container").height() - y + obj.find(".date-content").height() ;
